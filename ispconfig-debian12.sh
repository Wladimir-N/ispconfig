...
DEBIAN_FRONTEND=noninteractive apt -y install --no-install-recommends cron ntp postfix-mysql postfix-doc mariadb-client mariadb-server getmail6 rkhunter binutils dovecot-imapd dovecot-pop3d dovecot-mysql dovecot-sieve dovecot-lmtpd sudo curl rsyslog gnupg2 lsb-release rspamd redis clamav clamav-daemon unzip arj nomarch lzop cabextract p7zip p7zip-full unrar lrzip libnet-ldap-perl libauthen-sasl-perl clamav-docs daemon libnet-ident-perl zip libdbd-mysql-perl postgrey php8.2 php8.2-fpm php8.2-common php8.2-gd php8.2-mysql php8.2-imap php8.2-cli php8.2-cgi php-pear mcrypt imagemagick libruby php8.2-curl php8.2-intl php8.2-pspell php8.2-sqlite3 php8.2-tidy php8.2-xmlrpc php8.2-xsl php-imagick php8.2-zip php8.2-mbstring php8.2-soap php8.2-opcache php-apcu php8.2-mcrypt pure-ftpd-common pure-ftpd-mysql fail2ban iptables nginx-light fcgiwrap certbot patch pwgen quota wget dnsmasq jailkit
echo -e "PrivateMirror https://clamav-mirror.ru/\nPrivateMirror https://mirror.truenetwork.ru/clamav/\nPrivateMirror http://mirror.truenetwork.ru/clamav/\nScriptedUpdates no" >> /etc/clamav/freshclam.conf

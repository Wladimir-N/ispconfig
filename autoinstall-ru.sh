...
if [ $exitstatus = 0 ]; then
    ...
    if [ $exitstatus = 0 ]; then
			...
			echo "deb https://mirror.yandex.ru/mirrors/packages.sury.org/php $(grep VERSION_CODENAME /etc/os-release | cut -d '=' -f 2) main" > /etc/apt/sources.list.d/php.list
			wget -q https://mirror.yandex.ru/mirrors/packages.sury.org/php/apt.gpg -O- | apt-key add -
			DEBIAN_FRONTEND=noninteractive apt update
			if [ $PHP = 8.0 ];then
